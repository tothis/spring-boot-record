...
# 需要quartz.properties文件中设置org.quartz.jobStore.driverDelegateClass = org.quartz.impl.jdbcjobstore.StdJDBCDelegate
# StdJDBCDelegate说明支持集群 所有任务信息都会保存到数据库中 可以控制事物 如果应用服务器关闭或者重启 任务信息都不会丢失 且可以恢复因服务器关闭或者重启而导致执行失败的任务
# This is the script from Quartz to create the tables in a MySQL database, modified to use INNODB instead of MYISAM.
# quartz脚本 在MySQL数据库中创建以下表 使用INNODB而不是MYISAM
...
-- Trigger作为Blob类型存储 用于Quartz用户用JDBC创建他们自己定制的Trigger类型 JobStore并不知道如何存储实例时
...
# 存储与已触发Trigger相关状态信息 以及相联Job执行信息
